...
export const LABS_DATA: LabInfo[] = [
     ...
     ========================================================================== */
  {
    ...
    nextDecision: {
      en: 'Adopted as the default behaviour of the current package. EXP-03-F is kept as-is on an isolated copy of the v0.2.0 engine (apps/showcase/src/labs/engine-v0.2/); this lab runs on a frozen copy of the v1.0 engine (engine-v1.0/).',
      ko: '현재 패키지의 기본 동작으로 채택. EXP-03-F는 v0.2.0 엔진 사본(apps/showcase/src/labs/engine-v0.2/)으로 격리해 그대로 남기고, 이 실험은 v1.0 엔진 사본(engine-v1.0/)에서 돈다.',
    },
  },
  {
    id: 'exp04_b',
    ...
    keyFinding: {
      ...
      ko: '스크롤 주인이 둘이라는 것이 문제의 전부였다. 첫 구현의 모든 버그 -- 블러 시 위치 리셋, 사이클당 15px 드리프트, 40px 밀림, 키보드가 열리며 죽는 탭 -- 는 문서와 셸 사이에서 좌표를 주고받는 동안 Safari가 조건을 바꿔서 생겼다. position: fixed로 문서를 잠그면 오프셋이 리셋되고 Safari가 손가락 아래에서 주소창을 다시 펼친다(스파이크 B: 탭 3/3 사망). overflow: hidden만 쓰면 오프셋은 남지만 Safari가 포커스 인풋을 향해 +507 밀고 터치가 뒤 페이지를 밀어낸다(스파이크 A). 문서 높이를 정확히 "스크롤 위치 + 뷰포트 하나"로 캡하고 overflow: hidden(스파이크 C)을 걸면 Safari가 밀 곳이 없고, 오프셋도 주소창도 그대로다. 그 위에서 모드는 EXP-04-A처럼 CSS(:focus-within / :has())가 결정하고, JS는 문서가 스크롤될 때 캡을 발행하고(엔진이 키보드 높이를 발행하는 방식 그대로) 진입 시 column-reverse 셸 스크롤러로 위치를 한 번 옮기는 일만 한다. 해제 시엔 아무것도 되돌려 쓰지 않는다. 탭 자체에 대한 실기기 발견 둘: pointerdown 포커스는 제스처 도중 레이아웃을 바꿔 인풋을 짚고 끌면 문서와 셸이 동시에 스크롤됐고, 캐럿을 키보드 뒤에 ~35ms 두면 iOS가 visual viewport를 스스로 밀어올린다. 그래서 pointerup에 잠그고, inset은 즉시 반영하며, 같은 탭이 다른 곳에 합성하는 mousedown은 blur 대신 거부한다. 발견 하나는 엔진까지 갔다: column-reverse 본문은 WebKit에서 scrollTop 0일 때만 하단 고정이고, 위로 스크롤된 상태에서 상자가 줄면 위쪽 기준 오프셋을 유지해 최신 내용이 키보드 뒤로 사라진다. 그래서 useMobileKeyboard가 플로팅 바 포커스 중엔 하단 가장자리를 붙잡는다. 출시된 레이아웃을 폰에서 쓰며 둘이 더 나왔다: 본문 인풋 blur의 유예창이 1초 안에 이어진 작성기 탭을 가로채 읽던 위치를 잃게 했고, 가려진 인풋을 scrollIntoView로 드러내면 키보드가 닫히는 중엔 문서까지 스크롤되어 헤더가 함께 밀렸다 -- 훅은 이제 유예창 안에서도 바에 차례를 넘기고, <main>만 스크롤해 드러낸다. 마지막 하나가 남은 보고 전부를 설명했다: iOS 사파리는 대부분의 키보드 열림에서 레이아웃 뷰포트를 키보드 높이의 일부 또는 전부만큼 줄이고(상단 인풋 innerHeight 695→601, 하단 인풋 695→396 측정), px로 얼린 캡이 남긴 여지 -- 94px, 299px -- 만큼 윈도우를 스크롤한다. 고정 셸이 함께 올라가 헤더가 사라지고, 작성기 자리에 문서 끝 너머의 까만 공백이 보이고, 하단 인풋이 숨고, blur 때 다른 오프셋이 돌아왔다. 캡은 이제 calc(offset + 100%): JS는 오프셋만 발표하고 뷰포트는 CSS의 100%가 따라가므로 사파리가 innerHeight를 어떻게 바꾸든 여지는 0이다. 그래도 18번 중 5번은 올라갔다: 사파리의 캐럿 리빌은 문서 자체의 최대치를 넘어 윈도우를 밀고(y = offset + 94, + 299), 주소창을 펼친 뒤엔 윈도우는 그대로 두고 시각 뷰포트만 40px 밀었다 -- overflow 규칙은 참조되지 않는다. 그래서 JS에 남은 한 가지 의무는 선언을 집행하는 것: 셸이 떠 있는 동안 윈도우가 발표한 오프셋을 벗어나면 되돌린다. 04-A의 lockWindowTop을 목표 0 대신 읽던 위치로 일반화하고, 350ms 타이머 대신 이벤트로 트리거한 것이다. 같은 트레이스에서 하나 더: 주소창이 접힌 상태에선 캡 자체가 포커스 순간 문서를 40px 밀어올렸다 -- iOS에서 html의 100%는 innerHeight가 735일 때도 작은 뷰포트(695)이기 때문. 캡은 이제 offset + 100lvh로 뷰포트보다 작아지는 일이 없고, 남는 여지는 가드가 닫는다. 그리고 같은 트레이스를 헤더가 아니라 inset으로 다시 읽으니, 열림의 ~40%에서 --rmkl-kb-inset이 키보드 303px에 대해 4px나 209px로 정착해 있었다. 사파리가 키보드와 함께 innerHeight를 줄였고, 가드가 윈도우를 되돌렸고, innerHeight가 돌아왔는데 -- iOS는 그때 resize를 보내지 않는다, scroll만 보낸다. 훅은 resize에서만 측정했으므로 헤더는 0인데 작성기와 하단 인풋은 키보드 뒤에 있었다. 훅은 이제 scroll에서도 측정한다.',
    },
    nextDecision: {
      en: 'All six criteria pass on an iPhone (iOS 18.7 Safari, in-page traces) and in the iOS Simulator (Appium, real software keyboard). Shipped as the package\'s PageLayout: the document scrolls until an input is tapped, then the shell takes over at the same reading position. SubpageLayout stays for pages that are a shell to begin with. The hook fixes this lab produced -- lockDurationMs: 0 turning the top-lock off, holding the bottom edge for the floating bar, revealing a hidden body input smoothly -- shipped with it, and the Playground runs on PageLayout. Not concluded yet: the lab renders the library\'s PageLayout from the engine copy (apps/showcase/src/labs/engine-v1.0/), refreshed as each finding lands, so lab and library stay one code until the device confirms it. The fifth round held the header at 0 in 10 of 10 opens (339px pans, collapsed URL bar, input under the header), but the same traces showed the inset stale in ~40% -- the verdict had checked the header and the offset, not the composer. The sixth round did: 48 opens judged on offset, header, visual-viewport offset, inset and the composer\'s bottom edge together -- 47 right, the one exception a 7px resting offset of the collapsed URL bar. What remains is not a state: when Safari pans (9 of 48), the guard pulls the shell back within 0-100ms, and that pull-back is visible as a flicker. Removing it means Safari never wanting to reveal -- reserving the last keyboard height at focus so the caret is above the keyboard before Safari checks -- which is a design question for a next lab, not a patch here. The layout ships as is; the lab is concluded on the sixth round.',
      ko: 'iPhone(iOS 18.7 Safari, 페이지 내 트레이스)과 iOS 시뮬레이터(Appium, 실제 소프트웨어 키보드)에서 6개 항목 전부 통과. 패키지의 PageLayout으로 출시: 인풋을 탭하기 전까지 문서가 스크롤되고, 탭하면 같은 읽기 위치에서 셸이 이어받는다. 처음부터 셸인 페이지에는 SubpageLayout이 남는다. 이 실험이 낳은 훅 수정 -- lockDurationMs: 0으로 탑락 끄기, 플로팅 바 포커스 중 하단 가장자리 유지, 가려진 본문 인풋을 부드럽게 드러내기 -- 도 함께 반영했고, 체험 탭은 PageLayout으로 돈다. 아직 완결 전: 랩은 엔진 사본(apps/showcase/src/labs/engine-v1.0/)의 PageLayout을 그대로 렌더하고, 파인딩이 들어갈 때마다 사본을 갱신해 실기기 확인까지 랩과 라이브러리를 한 코드로 유지한다. 다섯 번째 라운드는 열림 10번 중 10번에서 헤더를 0에 붙잡았지만(339px 밀림, 접힌 주소창, 헤더 밑 인풋 포함), 같은 트레이스에서 inset이 ~40% 낡아 있었다 -- 판정이 헤더와 오프셋만 봤고 작성기를 안 봤다. 여섯 번째 라운드가 그랬다: 열림 48번을 오프셋·헤더·시각 뷰포트 오프셋·inset·작성기 바닥으로 함께 판정해 47번 정상, 예외 하나는 접힌 주소창의 7px 안정 오프셋. 남은 것은 상태가 아니다: 사파리가 밀 때(48번 중 9번) 가드가 0~100ms 안에 셸을 되돌리는데, 그 되돌림이 깜박임으로 보인다. 이를 없애려면 사파리가 드러낼 필요를 못 느끼게 -- 포커스 시점에 마지막 키보드 높이를 미리 확보해 사파리가 확인하기 전에 캐럿을 키보드 위에 두는 것 -- 해야 하고, 이는 패치가 아니라 다음 실험의 설계 질문이다. 레이아웃은 이대로 출시하고, 랩은 여섯 번째 라운드에서 완결한다.',
    },
  },
]
