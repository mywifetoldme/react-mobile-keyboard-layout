...
export const LABS_DATA: LabInfo[] = [
  ...
  },
  {
    ...
    nextDecision: {
      en: 'All six criteria pass on an iPhone (iOS 18.7 Safari, in-page traces) and in the iOS Simulator (Appium, real software keyboard). Kept as the reference pattern for pages that need the document to scroll; the package itself stays app-shell-first (EXP-04-A), and the two hook fixes this lab produced -- lockDurationMs: 0 turning the top-lock off, and holding the bottom edge for the floating bar -- shipped into it. EXP-04-A and EXP-04-B are frozen on a copy of this engine (apps/showcase/src/labs/engine-v1.0/) so the package can keep moving.',
      ko: 'iPhone(iOS 18.7 Safari, 페이지 내 트레이스)과 iOS 시뮬레이터(Appium, 실제 소프트웨어 키보드)에서 6개 항목 전부 통과. 문서 스크롤이 필요한 페이지의 기준 패턴으로 남긴다. 패키지 자체는 앱 셸 우선(EXP-04-A)을 유지하고, 이 실험이 낳은 훅 수정 둘 -- lockDurationMs: 0으로 탑락 끄기, 플로팅 바 포커스 중 하단 가장자리 유지 -- 는 패키지에 반영했다. EXP-04-A/B는 이 엔진의 사본(apps/showcase/src/labs/engine-v1.0/)에 고정해 패키지가 계속 움직일 수 있게 한다.',
    },
  },
]
