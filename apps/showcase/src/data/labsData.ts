...
export const LABS_DATA: LabInfo[] = [
  ...
  },
  {
    ...
    hypothesis: {
      en: 'Tracking visualViewport.offsetTop with hardware-accelerated transform: translateY will eliminate visual jank without scroll locks.',
      ko: '스크롤을 막지 않고 오프셋을 읽어와 translateY로 컨테이너를 가시 영역에 1:1 따라가게 만든다.',
    },
    evaluations: [
      { id: '1-1', status: 'fail', comment: { en: 'Stuttering follow lag: Container jitters and chases offset when rubbing input bar', ko: '인풋 바를 문지를 때 화면이 드르륵거리며 덜컹거리며 쫓아가는 래그 현상 발생' } },
    ...
    keyFinding: {
      ...
      ko: '탈락: 인풋 바를 문지르면 인풋 아래 거대한 빈 공간이 뚫린 채 화면이 드르륵거리며 쫓아가고, 포커스 해제 시 밑에서부터 화면이 솟아오르는 등 "사후 오프셋 추적"의 물리적 한계 노출.',
    },
    nextDecision: {
      en: 'Permanently abandon post-event offset tracking and commit 100% to pre-emptive top: 0 anchor lock (02-B / 02-D).',
      ko: '오프셋 따라가기 방식을 영구 폐기하고, 도화지 자체를 미리 top:0에 묶어두는 사전 차단(02-B / 02-D)을 최종 방향으로 확정.',
    },
  },
  {
    id: 'exp02_d',
