...
export const LABS_DATA: LabInfo[] = [
  ...
  },
  {
    ...
    keyFinding: {
      en: 'Two scroll owners are the whole problem. Every bug the first build had -- the position reset on blur, the 15px-per-cycle drift, the 40px push, the tap that died as the keyboard opened -- was a coordinate handed from the document to the shell and back while Safari changed the terms in between. Locking the document with position: fixed resets its offset and makes Safari re-expand its URL bar under the finger (spike B: 3/3 taps died); overflow: hidden alone keeps the offset but Safari pans +507 for the focused input and touch still scrolls the page (spike A). Capping the document at exactly "scroll position + one viewport" with overflow: hidden (spike C) leaves Safari nothing to pan, keeps the offset, and keeps the URL bar as it was. With that, the mode is decided in CSS (:focus-within / :has()) as in EXP-04-A, and JS does one thing: publish the cap and carry the position into the column-reverse shell scroller, once, at lock time. Nothing is written back on unlock. Two more device findings shaped the tap itself: focusing at pointerdown swapped the layout mid-gesture (a drag from an input scrolled the document and the shell at once) and iOS pans the visual viewport itself if the caret is left under the keyboard for ~35ms, so the tap locks at pointerup, the inset applies at once, and the mousedown the same tap synthesizes elsewhere is refused instead of blurring the input. One finding reached the engine: a column-reverse body is bottom-anchored by WebKit only at scrollTop 0 -- scrolled up, a shrinking box keeps its top-based offset and the newest content slides behind the keyboard -- so useMobileKeyboard now holds the bottom edge while the floating bar has the focus.',
      ko: '스크롤 주인이 둘이라는 것이 문제의 전부였다. 첫 구현의 모든 버그 -- 블러 시 위치 리셋, 사이클당 15px 드리프트, 40px 밀림, 키보드가 열리며 죽는 탭 -- 는 문서와 셸 사이에서 좌표를 주고받는 동안 Safari가 조건을 바꿔서 생겼다. position: fixed로 문서를 잠그면 오프셋이 리셋되고 Safari가 손가락 아래에서 주소창을 다시 펼친다(스파이크 B: 탭 3/3 사망). overflow: hidden만 쓰면 오프셋은 남지만 Safari가 포커스 인풋을 향해 +507 밀고 터치가 뒤 페이지를 밀어낸다(스파이크 A). 문서 높이를 정확히 "스크롤 위치 + 뷰포트 하나"로 캡하고 overflow: hidden(스파이크 C)을 걸면 Safari가 밀 곳이 없고, 오프셋도 주소창도 그대로다. 그 위에서 모드는 EXP-04-A처럼 CSS(:focus-within / :has())가 결정하고, JS는 캡을 발행하고 column-reverse 셸 스크롤러로 위치를 한 번 옮기는 일만 한다. 해제 시엔 아무것도 되돌려 쓰지 않는다. 탭 자체에 대한 실기기 발견 둘: pointerdown 포커스는 제스처 도중 레이아웃을 바꿔 인풋을 짚고 끌면 문서와 셸이 동시에 스크롤됐고, 캐럿을 키보드 뒤에 ~35ms 두면 iOS가 visual viewport를 스스로 밀어올린다. 그래서 pointerup에 잠그고, inset은 즉시 반영하며, 같은 탭이 다른 곳에 합성하는 mousedown은 blur 대신 거부한다. 발견 하나는 엔진까지 갔다: column-reverse 본문은 WebKit에서 scrollTop 0일 때만 하단 고정이고, 위로 스크롤된 상태에서 상자가 줄면 위쪽 기준 오프셋을 유지해 최신 내용이 키보드 뒤로 사라진다. 그래서 useMobileKeyboard가 플로팅 바 포커스 중엔 하단 가장자리를 붙잡는다.',
    },
    nextDecision: {
      en: 'Verified in the iOS Simulator through Appium with a real software keyboard: the document stays at its offset through five keyboard cycles at three positions, the header stays at 0, the composer sits flush on the keyboard, and the body-input tap that used to die survives. Confirmed on an iPhone (iOS 18.7 Safari) through in-page traces: the document offset holds at 0 and at 2578 across every cycle, the URL bar is left alone, the bottom body input keeps the keyboard, and a drag that starts on an input never locks.',
      ko: 'Appium으로 iOS 시뮬레이터에서 실제 소프트웨어 키보드로 검증: 세 위치에서 다섯 번의 키보드 사이클 동안 문서 오프셋 유지, 헤더 0 고정, 작성기 키보드 상단 밀착, 그리고 이전에 즉사하던 본문 인풋 탭 생존. iPhone(iOS 18.7 Safari) 실기기 페이지 내 트레이스로 확인: 모든 사이클에서 문서 오프셋 0·2578 유지, 주소창 불변, 최하단 본문 인풋 키보드 유지, 인풋에서 시작한 끌기는 잠기지 않음.',
    },
  },
]
