...
**v2.0.0부터 이 사본은 `SubpageLayout`과 `useMobileKeyboard`(EXP-04-A의 셸과 엔진)가 남아 있는 유일한 곳입니다.**
패키지에서는 제거되었고(04-B가 이 셸에 보통 페이지를 두른 것이라 채팅 화면까지 덮음), EXP-03-D/E와 04-A는 여기서 계속 돕니다.
EXP-04-B는 `engine-v2.0/`(패키지 2.0.0의 사본)에서 돕니다.

## 출처

- 커밋: `72d82ae` (v1.0.0 + PR #23: `PageLayout`, `FloatingInput` pointerup 포커스, `useMobileKeyboard` 수정들)
  ...
  - 탭의 click 대기창(600ms)과 본문 인풋 blur 유예(1000ms)를 시간이 아니라 이벤트로 닫음 (다음 pointerdown/click, 다음 키보드 인풋 focusin)
...
- 복사한 파일: `components/SubpageLayout.tsx` `components/SubpageLayout.css`
  ...
  `hooks/useMobileKeyboard.ts` `hooks/usePageKeyboard.ts` `utils/isKeyboardTextInput.ts` `index.ts`
...
for p in components/SubpageLayout.tsx components/SubpageLayout.css \
         components/PageLayout.tsx components/PageLayout.css \
         components/FloatingInput.tsx components/FloatingInput.css \
         hooks/useMobileKeyboard.ts hooks/usePageKeyboard.ts utils/isKeyboardTextInput.ts index.ts; do
  git show "72d82ae:packages/react-mobile-keyboard-layout/src/$p" \
